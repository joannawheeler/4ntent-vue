  ...
  <div class="availability-container">
    <div class="row no-gutters">
      <div class="col-xs-12 centered">
        <div class="view-title"><span style="color: black !important">Book appointments with your mentors here</span></div>
      </div>
    </div>
    <div class="row no-gutters">
      <div class="col-xs-12 centered">
        <v-select class="my-2" label="Appointment Type" v-model="curAppointment" :items="appointmentsList" target="#dropdown-example" return-object v-on:change="getUsersUnscheduledAppts"></v-select>
    ...
    <div class="row no-gutters">
      <div class="col-xs-12 centered">
        <v-select class="my-2" label="Select a mentor" v-model="curMentor" :items="userinfo.mentors" target="#dropdown-example" return-object v-on:change="getCurMentorsAvilability"></v-select>
        <!--         <v-select class="my-2" :disabled="day.disabled" v-model="day" :items="days" target="#dropdown-example" return-object v-on:change="getIntervals"></v-select> -->
      </div>
    </div>
    <div class="row no-gutters">
    ...
    <div class="row no-gutters">
      <div class="col-xs-12 centered">
        <div class="slots"><span style="color: black !important">Your available slots:</span></div>
      </div>
    </div>
    <div class="row no-gutters">
      <div class="col-xs-12 centered">
...
export default {
  ...
  created() {
    ...
    db.collection('users').doc(this.id).get()
      .then(doc => {
        ...
        this.curMentor = userinfo.mentors[0]
        return userinfo
      }).then(userinfo => {
        if (userinfo.tier === "passion") {

        } else if (userinfo.tier === "purpose") {

        } else if (userinfo.tier === "empower") {

        }
        this.appointmentsList = [];
        this.curAppointment = '';
        this.appointmentsToBook = userinfo.appointmentsToBook;
        //
        // appointmentsToBook: {
        // 'Weekly Wellness Group': {
        // weekRangesNotYetBooked: {}
        // }
        // , 'Custom 30 min Session with Your Designated Health Mentor', '30 min 1 on 1 Individual Session with a Health Mentor of Your Choice'
        // }
  ...
  methods: {
    moment,
    getUsersUnscheduledAppts() {

    },
    getCurMentorsAvilability() {

    },
    setCurMentorsAvilability() {
