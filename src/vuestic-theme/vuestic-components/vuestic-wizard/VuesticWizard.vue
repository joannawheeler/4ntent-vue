<template>
  <div class="wizard" :class="computedLayout" v-orientation-handler="{ layout: wizardLayout,  breakPoint: orientationBreakPoint }">
    <div v-if="computedLayout === 'horizontal'" class="indicator-container indicator-container-horizontal">
      <simple-horizontal-indicator v-if="wizardType === 'simple'" :steps="steps" :currentStep="currentStep" :completed="wizardCompleted" />
      <rich-horizontal-indicator v-if="wizardType === 'rich'" :steps="steps" :currentStep="currentStep" :completed="wizardCompleted" />
    </div>
    <div v-if="computedLayout === 'vertical'" class="indicator-container indicator-container-vertical">
      <rich-vertical-indicator v-if="wizardType === 'rich'" :steps="steps" :currentStep="currentStep" :completed="wizardCompleted" />
      <simple-vertical-indicator v-if="wizardType === 'simple'" :steps="steps" :currentStep="currentStep" :completed="wizardCompleted" />
    </div>
    <div class="wizard-body">
      <div class="wizard-body-step" v-for="(step, index) in steps" :key="index" v-show="isStepShown(step)">
...
export default {
  ...
  methods: {
    ...
    goNext() {
      ...
      //if the user is logged in, skip the second step where they would normally sign up
      if (this.$store.state['app']['isLoggedIn'] && this.currentStep === 0) {
        this.currentStep = this.currentStep + 2;
        //if the user is NOT logged in or the current step is not 0
      } else {
        //if this is NOT the last step and the step is valid...
        if (!this.isLastStep() && this.isCurrentStepValid()) {
          //if the current step is 1 (the signup step)
          if (this.currentStep === 1) {
            //check if the input is unique
            this.steps[this.currentStep].isUnique(function(bool) {
              console.log(bool)
              if (bool === false) {
                console.log("is not unique")
                alert("Oops, that email is associated with an existing account. Please login instead!")
                return false
              } else {
            ...
            //if we are on any other step, move on to next step (after all the other checks have passed)
          } else {
            this.currentStep++;
          }
        }
