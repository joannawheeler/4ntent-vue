<template>
  <transition name="va-modal__overlay__transition" appear :duration="withoutTransitions ? 0 : 200">
    <div v-if="overlayValue" class="va-modal__overlay" :class="computedOverlayClass" @click="checkOutside" :style="computedOverlayStyles">
      <transition name="va-modal__transition" appear :duration="withoutTransitions ? 0 : 500">
        <div v-if="value" class="va-modal" :class="computedClass" :style="{maxWidth, maxHeight}">
          <i v-if="fullscreen" @click="cancel" class="ion ion-md-close va-modal__close" />
          <div class="va-modal__inner" :style="{maxHeight, maxWidth}">
            <div v-if="title" class="mb-4 title" :style="{color: this.$themes.primary}">
              {{title}}
            </div>
